...
func main() {
	...
	// REDIS_PASSWORD hard-fail is enforced inside `config.Load()` via
	// `requireEnv` (#34) — by the time we get here, RedisPassword is
	// guaranteed non-empty. See `internal/config/config.go::requireEnv`
	// for the rationale on why transitive `Ping → NOAUTH` isn't enough.
